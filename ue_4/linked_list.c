...
// Recursive version
void list_insert_end(node_t* * pnode, int data) {
	node_t* node = *pnode;
	// Is the list empty
	if (node == NULL) {
		*pnode = list_create_node(data);
	} else {
		// Recursive call
		list_insert_end(&node->next, data);
		*pnode = node;
	}
}	
...
// Recursive version
void list_contains(node_t** pnode, int data, bool* res) {
	node_t* node = *pnode;
	if (node == NULL) {
		*res = false;
		...
		*res = true;
		...
		// Recursive call
		list_contains(&node->next, data, res);
	}
}
